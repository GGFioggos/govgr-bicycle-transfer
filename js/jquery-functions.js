$("document").ready(function () {
    ...
    function skipToEnd(message) {
        ...
        const error =
            currentLanguage === "greek"
                ? "Λυπούμαστε αλλά δεν δικαιούστε άδεια μεταβίβασης μοτοποδηλάτου!"
    ...
    function retrieveAnswers() {
        var allAnswers = [];
        getEvidencesById(1); // Private agreement
        for (var i = 0; i < totalQuestions; i++) {
            var answer = sessionStorage.getItem("answer_" + i);
            allAnswers.push(answer);
        }

        if (allAnswers[0] === "1") { // Agreement is available
            getEvidencesById(1);
        }
        if (allAnswers[1] === "1") { // Registration is available
            getEvidencesById(2);
        }
        if (allAnswers[2] === "1") { // Identification documents available
            getEvidencesById(3);
        }
        if (allAnswers[3] === "1") { // Fees paid
            getEvidencesById(4);
        }
        if (allAnswers[4] === "1") { // Technical requirements met
            currentLanguage === "greek"
                ? setResult("Όλα τα απαιτούμενα δικαιολογητικά και προϋποθέσεις καλύφθηκαν.")
                : setResult("All necessary documents and requirements are met.");
        } else {
            skipToEnd(
                currentLanguage === "greek"
                    ? "Το μοτοποδήλατο δεν πληροί τις τεχνικές προϋποθέσεις."
                    : "The moped does not meet the technical requirements."
            );
        }
    }
    ...
    $("#nextQuestion").click(function () {
        if ($(".govgr-radios__input").is(":checked")) {
            ...
            // Updated logic with new messages for bicycle transfer
            if (currentQuestion === 0 && selectedRadioButtonIndex === 2) {
                currentQuestion = -1;
                currentLanguage === "greek"
                    ? skipToEnd("Πρέπει να έχετε ιδιωτικό συμφωνητικό πώλησης ποδηλάτου με επικυρωμένη υπογραφή.")
                    : skipToEnd("You must have a private agreement for the sale of the bicycle with validated signatures.");
            } else if (currentQuestion === 1 && selectedRadioButtonIndex === 2) {
                currentQuestion = -1;
                currentLanguage === "greek"
                    ? skipToEnd("Η άδεια κυκλοφορίας του ποδηλάτου είναι απαραίτητη για τη μεταβίβαση.")
                    : skipToEnd("The registration document of the bicycle is required for the transfer.");
            } else if (currentQuestion === 2 && selectedRadioButtonIndex === 2) {
                currentQuestion = -1;
                currentLanguage === "greek"
                    ? skipToEnd("Πρέπει να παρέχετε ταυτοποιητικά έγγραφα τόσο του αγοραστή όσο και του πωλητή.")
                    : skipToEnd("You must provide identification documents for both the buyer and the seller.");
            } else if (currentQuestion === 3 && selectedRadioButtonIndex === 2) {
                currentQuestion = -1;
                currentLanguage === "greek"
                    ? skipToEnd("Πρέπει να καταβληθούν τα απαραίτητα παράβολα για τη μεταβίβαση.")
                    : skipToEnd("The required fees for the transfer must be paid.");
            } else if (currentQuestion === 4 && selectedRadioButtonIndex === 2) {
                currentQuestion = -1;
                currentLanguage === "greek"
                    ? skipToEnd("Το ποδήλατο πρέπει να πληροί τις τεχνικές προϋποθέσεις για μεταβίβαση.")
                    : skipToEnd("The bicycle must meet the technical requirements for transfer.");
            } else {
                // Save selectedRadioButtonIndex to storage
                ...
                // If the questions are finished, submit the form
